    ...
    U: nmea::nmea2000::Raw + nmea::nmea2000::From<String> + Send,
{
    for line in reader.lines() {
        if let Some(message) = parser
            .parse(&line.context("error processing line")?)
...
fn main() -> Result<()> {
    ...
    if !reading_from_file {
        ...
        let writer_handle =
            thread::spawn(move || write_thread(&mut writer, writer_state, opt.interval));

        let reader_state = Arc::clone(&state_arc);
        let reader_handle = thread::spawn(move || read_thread(reader, &mut parser, reader_state));
