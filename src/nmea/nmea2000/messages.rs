...
impl nmea2000::Message for PositionRapidUpdateMessage {
    ...
    fn values(&self) -> Vec<MessageValue> {
        let mut lat =
            i32::from_le_bytes([self.data[0], self.data[1], self.data[2], self.data[3]]) as f32;
        lat *= 0.0000001;

        let mut long =
            i32::from_le_bytes([self.data[4], self.data[5], self.data[6], self.data[7]]) as f32;
        long *= 0.0000001;
...
impl nmea2000::Message for GNSSPositionData {
    ...
    fn values(&self) -> Vec<MessageValue> {
        ...
        let mut time =
            u32::from_le_bytes([self.data[3], self.data[4], self.data[5], self.data[6]]) as f32;
        time *= 0.0001;
        //Latitude
...
impl nmea2000::Message for VesselHeadingMessage {
    ...
    fn values(&self) -> Vec<MessageValue> {
        ...
        vec![Heading(F16(hdg)), Timestamp(self.timestamp)]
    }
}
...
impl nmea2000::Message for SpeedMessage {
    ...
    fn values(&self) -> Vec<MessageValue> {
        ...
        vec![SpeedThroughWater(F16(stw)), Timestamp(self.timestamp)]
    }
}
...
impl nmea2000::Message for RateOfTurnMessage {
    ...
    fn values(&self) -> Vec<MessageValue> {
        let rot = i32::from_le_bytes([self.data[1], self.data[2], self.data[3], self.data[4]])
            as f32
            * 3.125e-08;
        vec![RateOfTurn(F32(rot)), Timestamp(self.timestamp)]
    }
}
...
impl nmea2000::Message for RudderMessage {
    ...
    fn values(&self) -> Vec<MessageValue> {
        ...
        vec![RudderAngle(F16(ra)), Timestamp(self.timestamp)]
    }
}
...
impl nmea2000::Message for TimeDateMessage {
    fn values(&self) -> Vec<MessageValue> {
        ...
        let time = u32::from_le_bytes([self.data[2], self.data[3], self.data[4], self.data[5]])
            as f32
            * 0.0001;
